...
let { isInputsCalculated, isFlashCalculated } = false;
...
factorCalculateButton.addEventListener('click', (event) => {
  ...
  if (isFlashCalculated === true && isInputsCalculated === true) {
...
sizingCalculateButton.addEventListener('click', (event) => {
  ...
  if (isFlashCalculated === true && isInputsCalculated === true) {
    ...
    const d_p_min = Math.sqrt(
      (18 * viscosityConverted * V_t_min) / ((rho_l - rho_g) * 32.2)
    ).toFixed(4);
